...
(() => {
  ...
  let board = null, action = null, drawing = [], drawingMode = false;
  ...
  const input = (label,key,value='',area=false) => { const wrap=document.createElement('div'); wrap.className='field'; const l=document.createElement('label'); l.textContent=label; const el=document.createElement(area?'textarea':'input'); el.name=key; el.value=value; wrap.append(l,el); return wrap; };
  ...
  form.addEventListener('submit',async e=>{e.preventDefault(); statusEl.textContent=''; try { await action(new FormData(form)); close(); await load(); } catch(error){statusEl.textContent=error.message;}});
  ...
  function render() {
    ...
    board.categories.forEach(category=>{ const col=document.createElement('section'); col.className='column'; const head=document.createElement('div'); head.className='column-head'; const name=document.createElement('span'); name.className='column-title'; name.textContent=esc(category.name); const count=document.createElement('span'); count.className='count'; count.textContent=category.cards.length; const tools=document.createElement('span'); tools.className='column-tools'; const rename=document.createElement('button'); rename.className='icon-btn'; rename.textContent='✎'; rename.title=t('rename'); rename.onclick=()=>categoryDialog(category); const remove=document.createElement('button'); remove.className='icon-btn'; remove.textContent='×'; remove.title=t('delete'); remove.onclick=()=>deleteCategory(category); tools.append(rename,remove); head.append(name,count,tools); col.append(head); category.cards.forEach(card=>col.append(makeTask(card))); const add=document.createElement('button'); add.className='add-card'; add.textContent=t('addCard'); add.onclick=()=>cardDialog(null,category); col.append(add); columnsEl.append(col); });
    draw();
  }
  ...
  let stroke=null; canvas.addEventListener('pointerdown',e=>{if(!drawingMode)return;stroke=[];drawing.push(stroke);const r=canvas.getBoundingClientRect();stroke.push([e.clientX-r.left,e.clientY-r.top]);canvas.setPointerCapture(e.pointerId)});canvas.addEventListener('pointermove',e=>{if(!stroke)return;const r=canvas.getBoundingClientRect();stroke.push([e.clientX-r.left,e.clientY-r.top]);draw()});canvas.addEventListener('pointerup',async()=>{if(stroke){stroke=null;await api(`/api/boards/${boardId}/drawing`,{method:'PUT',body:JSON.stringify({drawing})})}});
  toggle.onclick=()=>{drawingMode=!drawingMode;toggle.classList.toggle('active',drawingMode);canvas.style.pointerEvents=drawingMode?'auto':'none';columnsEl.style.pointerEvents=drawingMode?'none':'auto';};
...
async def create_category(request: Request, board_id: str):
    ...
    category = {"id": category_id, "name": name, "color": str(payload.get("color", "#5865f2"))[:20], "position": len(board["categories"]), "cards": {}}
