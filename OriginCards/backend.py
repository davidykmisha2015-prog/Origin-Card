...
(() => {
  ...
  function render() {
    ...
    board.categories.forEach(category=>{ const col=document.createElement('section'); col.className='column'; const head=document.createElement('div'); head.className='column-head'; const name=document.createElement('span'); name.className='column-title'; name.textContent=esc(category.name); const count=document.createElement('span'); count.className='count'; count.textContent=category.cards.length; const tools=document.createElement('span'); tools.className='column-tools'; const rename=document.createElement('button'); rename.className='icon-btn'; rename.textContent='✎'; rename.title=t('rename'); rename.onclick=()=>categoryDialog(category); const remove=document.createElement('button'); remove.className='icon-btn'; remove.textContent='×'; remove.title=t('delete'); remove.onclick=()=>deleteCategory(category); tools.append(rename,remove); head.append(name,count,tools); col.append(head);     category.cards.forEach(card=>col.append(makeTask(card))); const add=document.createElement('button'); add.type='button'; add.className='add-card'; add.textContent=t('addCard'); add.onclick=e=>{e.preventDefault();e.stopPropagation();cardDialog(null,category)}; col.append(add); columnsEl.append(col); });
    draw();
  }
